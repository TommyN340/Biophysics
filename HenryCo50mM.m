...
K = .7422;      %This is the Kappa value for the sample of interest
                %Next line extracts data from excel sheet and stores to
                %variable T
ZSS = xlsread("POPS_Chol in 50mM NaCl.xlsx","ZS (Sonic)");
ZPS = xlsread("POPS_Chol in 50mM NaCl.xlsx","ZP (Sonic)");
ZSPS= xlsread("POPS_Chol in 50mM NaCl.xlsx","ZS (Presonic)");
ZPPS= xlsread("POPS_Chol in 50mM NaCl.xlsx","ZP (Presonic)");

%%
a = ZSS(:,17);   %Sizes the T matrix down to just column 14 (or whatever column corresponds to #/2)
