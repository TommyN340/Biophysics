...
K = 1.0447;      %This is the Kappa value for the sample of interest
                %Next line extracts data from excel sheet and stores to
                %variable T
ZSS = xlsread("POPS_Chol in 100mM NaCl.xlsx","ZS (Sonic)");
ZPS = xlsread("POPS_Chol in 100mM NaCl.xlsx","ZP (Sonic)");
ZSPS= xlsread("POPS_Chol in 100mM NaCl.xlsx","ZS (Presonic)");
ZPPS= xlsread("POPS_Chol in 100mM NaCl.xlsx","ZP (Presonic)");

%%
a = ZSS(:,10);   %Sizes the T matrix down to just column 14 (or whatever column corresponds to #/2)
...
a2 = ZSPS(:,10);   %Sizes the T matrix down to just column 14 (or whatever column corresponds to #/2)
                %Removes all the NaN cells in the matrix
a2 = a2(isfinite(a2(:, 1)), :);
                %Next for-loop applies the Kappa values to the a values and
                %stores it in a new matrix with variable x. Values in x
